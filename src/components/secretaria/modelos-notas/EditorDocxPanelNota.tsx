...
export function EditorDocxPanelNota({
  modeloId,
  tiposNota,
  onModeloGuardado,
  showMessage,
  ...
  useEffect(() => {
    if (editor && modalGuardar) {
      ...
      setTipoGuardar(editor.tipoNotaId);
    }
  }, [modalGuardar, editor]);
      ...
      const res = await fetch(
        `/api/secretaria/modelos-nota/${modeloId}/contenido`,
        {
          ...
          body: JSON.stringify({
            ...
            tipoNotaId: modoGuardar === "nuevo" ? tipoGuardar : editor.tipoNotaId,
        ...
        <div className="fixed inset-0 z-[60] bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm border">
            ...
            <div className="p-5 space-y-3">
              <label
                className="flex items-start gap-3 cursor-pointer p-3 rounded-lg border-2 transition-colors hover:border-emerald-300 hover:bg-emerald-50"
                style={{
                  borderColor:
              ...
              <label
                className="flex items-start gap-3 cursor-pointer p-3 rounded-lg border-2 transition-colors hover:border-emerald-300 hover:bg-emerald-50"
                style={{
                  borderColor: modoGuardar === "nuevo" ? "#059669" : "#e5e7eb",
                }}
              >
                <input
                  type="radio"
