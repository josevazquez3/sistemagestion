...
type CategoriasOrdenDiaPanelProps = {
  canEdit: boolean;
  showMessage: (type: "ok" | "error", text: string) => void;
  onCategoriasChange: () => void;
};

export function CategoriasOrdenDiaPanel({
  canEdit,
  showMessage,
  onCategoriasChange,
}: CategoriasOrdenDiaPanelProps) {
  const [abierto, setAbierto] = useState(false);
  const [categorias, setCategorias] = useState<CategoriaOrdenDia[]>([]);
  const [loading, setLoading] = useState(false);
  const [modalNueva, setModalNueva] = useState(false);
  const [modalEditar, setModalEditar] = useState(false);
  const [categoriaEditar, setCategoriaEditar] = useState<CategoriaOrdenDia | null>(null);
  ...
  const fetchCategorias = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/secretaria/orden-del-dia/categorias");
  ...
  const handleActualizar = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/secretaria/orden-del-dia/categorias/${categoriaEditar.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nombre: nombre.trim(), descripcion: descripcion.trim() || null }),
      });
  ...
  const toggleActivo = async (c: CategoriaOrdenDia) => {
    try {
      const res = await fetch(`/api/secretaria/orden-del-dia/categorias/${c.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleEliminar = async (c: CategoriaOrdenDia) => {
    ...
    try {
      const res = await fetch(`/api/secretaria/orden-del-dia/categorias/${c.id}`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) {
