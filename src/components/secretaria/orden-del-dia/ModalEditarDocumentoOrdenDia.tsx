...
type ModalEditarDocumentoOrdenDiaProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  documento: DocumentoOrdenDia | null;
  categorias: CategoriaOrdenDia[];
  onSuccess: () => void;
  showMessage: (type: "ok" | "error", text: string) => void;
};

export function ModalEditarDocumentoOrdenDia({
  open,
  onOpenChange,
  documento,
  categorias,
  onSuccess,
  showMessage,
}: ModalEditarDocumentoOrdenDiaProps) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/secretaria/orden-del-dia/${documento.id}`, { method: "PUT", body: form });
