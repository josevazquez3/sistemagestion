...
export function ModalDuplicarModelo({
  modelo,
  tiposOficio,
  onClose,
  onGuardado,
  ...
  const [tipoOficioId, setTipoOficioId] = useState<number>(modelo.tipoOficioId);
  ...
  const handleGuardar = async () => {
    ...
    setError("");
    try {
      const res = await fetch(`/api/legales/modelos-oficio/${modelo.id}/duplicar`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nombre: nombreTrim, tipoOficioId }),
      });
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <select
              value={tipoOficioId}
              onChange={(e) => setTipoOficioId(parseInt(e.target.value, 10))}
