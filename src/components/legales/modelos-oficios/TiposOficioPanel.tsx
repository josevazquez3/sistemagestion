...
export function TiposOficioPanel({
  tipos,
  loading,
  onRefresh,
  onModelosRefresh,
  showMessage,
}: TiposOficioPanelProps) {
  ...
  const openEdit = useCallback((t: TipoOficio) => {
    setEditId(t.id);
    setNombre(t.nombre);
  ...
  const save = useCallback(async () => {
    ...
    try {
      const url = editId
        ? `/api/legales/tipos-oficio/${editId}`
        : "/api/legales/tipos-oficio";
  ...
  const toggleActivo = useCallback(
    async (t: TipoOficio) => {
      try {
        const res = await fetch(`/api/legales/tipos-oficio/${t.id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
    ...
    <>
      <Card>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <CardTitle>Tipos de Oficio</CardTitle>
          </div>
          <Button
            size="sm"
                  ...
                  <TableBody>
                    {tipos.length === 0 ? (
                      <TableRow>
                        ...
                        >
                          No hay tipos de oficio. Creá uno con &quot;Nuevo tipo de oficio&quot;.
                        </TableCell>
                      </TableRow>
                    ) : (
                      tipos.map((t) => (
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editId ? "Editar tipo de oficio" : "Nuevo tipo de oficio"}
            </DialogTitle>
          ...
          <div className="space-y-4 py-2">
            <div>
              ...
              <Input
                ...
                placeholder="Nombre del tipo de oficio"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
