...
type ModalEditarModeloOficioProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  modelo: ModeloOficio | null;
  tipos: TipoOficio[];
  onSuccess: () => void;
  showMessage: (type: "ok" | "error", text: string) => void;
};

export function ModalEditarModeloOficio({
  open,
  onOpenChange,
  modelo,
  tipos,
  ...
  useEffect(() => {
    if (modelo) {
      ...
      setTipoId(String(modelo.tipoOficioId));
      setFile(null);
    }
  }, [modelo]);
  ...
  const save = async () => {
    ...
    try {
      ...
      form.set("tipoOficioId", tipoId);
      if (file && file.size > 0) form.set("file", file);
      const res = await fetch(`/api/legales/modelos-oficio/${modelo.id}`, {
        method: "PUT",
        body: form,
      });
