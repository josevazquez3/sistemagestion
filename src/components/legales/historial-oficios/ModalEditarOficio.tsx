...
export function ModalEditarOficio({
  open,
  onOpenChange,
  oficio,
  onSuccess,
  showMessage,
}: ModalEditarOficioProps) {
  ...
  useEffect(() => {
    if (oficio) {
      ...
      setFechaOficio(formatFecha(oficio.fechaOficio));
  ...
  const save = async () => {
    ...
    if (!d || !m || !y) {
      showMessage("error", "La fecha del oficio es obligatoria (DD/MM/YYYY).");
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`/api/legales/historial-oficios/${oficio.id}`, {
        method: "PUT",
        body: form,
      });
      ...
      showMessage("ok", "Oficio actualizado correctamente.");
  ...
  const tieneArchivo = oficio.urlArchivo && oficio.nombreArchivo && !quitarArchivo;
          ...
          {oficio.urlArchivo && oficio.nombreArchivo && (
            <div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-sm text-gray-600">{oficio.nombreArchivo}</span>
                {!quitarArchivo && (
                  <Button
                    type="button"
                    size="sm"
