...
interface EditoresDocxStore {
  editores: EditorAbierto[];
  abrirEditor: (datos: {
    modeloId: number;
    nombre: string;
    tipoOficio: string;
  ...
  setCargando: (modeloId: number, valor: boolean) => void;
  marcarGuardado: (modeloId: number) => void;
}

const MAX_EDITORES = 3;

export const useEditoresDocxStore = create<EditoresDocxStore>((set, get) => ({
  editores: [],
  ...
  setCargando: (modeloId, valor) =>
    set((state) => ({
      editores: state.editores.map((e) =>
        e.modeloId === modeloId ? { ...e, cargando: valor } : e
      ),
    })),
