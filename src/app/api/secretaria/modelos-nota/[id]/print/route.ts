...
export async function GET(
  ...
  try {
    ...
    const titulo = modelo.nombre ?? modelo.nombreArchivo ?? "Modelo de nota";
    ...
    if (esWordDocBinario(modelo.nombreArchivo)) {
      htmlContenido = `<div style="padding:2rem;font-family:Arial,sans-serif;max-width:560px;">
        ...
        <p style="margin-top:0.75rem;">Descargá el modelo desde la tabla para abrirlo en Microsoft Word.</p>
      </div>`;
    } else {
...
<body>
  ...
  <script>window.addEventListener('load', () => { window.print(); });</script>
    ...
    console.error("Error convirtiendo DOCX (modelos nota):", error);
