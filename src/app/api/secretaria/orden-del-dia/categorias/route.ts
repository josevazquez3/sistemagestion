...
export async function GET(req: NextRequest) {
  ...
  const categorias = await prisma.categoriaOrdenDia.findMany({
    where,
    orderBy: { nombre: "asc" },
    include: {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const categoria = await prisma.categoriaOrdenDia.create({
      data: {
        nombre: nombre.trim(),
    ...
    console.error("Error creando categoría orden del día:", e);
    return NextResponse.json(
