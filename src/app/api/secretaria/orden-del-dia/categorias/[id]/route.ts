...
export async function PUT(
  ...
  const existente = await prisma.categoriaOrdenDia.findUnique({
    where: { id },
  });
  if (!existente) {
  ...
  try {
    ...
    const categoria = await prisma.categoriaOrdenDia.update({
      where: { id },
      data,
    });
    ...
    console.error("Error actualizando categoría orden del día:", e);
...
export async function DELETE(
  ...
  const categoria = await prisma.categoriaOrdenDia.findUnique({
    where: { id },
  ...
  await prisma.categoriaOrdenDia.delete({ where: { id } });
