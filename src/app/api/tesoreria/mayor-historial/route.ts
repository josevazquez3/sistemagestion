...
import { canAccess, ensureMayorHistorialTables, getRolesFromSession } from "./_shared";
...
export async function GET(req: NextRequest) {
  ...
  await ensureMayorHistorialTables();
  ...
  try {
    ...
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Error al listar historial" },
      { status: 500 }
    );
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    >(
      `INSERT INTO "HistorialMayorCuentas" ("nombreArchivo","fechaArchivo","sheetName","sheetData")
    ...
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Error al importar Excel" },
      { status: 500 }
    );
  }
}
