...
import { canAccess, ensureMayorHistorialTables, getRolesFromSession, parseId } from "../_shared";
...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Error al editar registro" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    const res = await prisma.$executeRawUnsafe(`DELETE FROM "HistorialMayorCuentas" WHERE "id" = $1`, id);
