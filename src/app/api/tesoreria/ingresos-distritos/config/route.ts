...
export async function GET(req: NextRequest) {
  ...
  const config = await prisma.configIngresoDistrito.findUnique({
...
export async function PUT(req: NextRequest) {
  ...
  const config = await prisma.configIngresoDistrito.upsert({
    where: { mes_anio: { mes, anio } },
  ...
  try {
    if (codigosOperativos.length > 0) {
      await registrarAuditoria({
        ...
        accion: `Configuró códigos Ingresos Distritos (${mes}/${anio}): ${codigosOperativos.join(", ")}`,
        modulo: "Tesorería",
      });
    }
  } catch {}
