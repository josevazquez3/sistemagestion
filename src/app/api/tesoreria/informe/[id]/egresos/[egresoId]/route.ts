...
export async function PUT(
  req: NextRequest,
  { params }: { params: Promise<{ id: string; egresoId: string }> }
) {
  ...
  const eid = parseId(egresoId);
  if (informeId == null || eid == null) {
  ...
  try {
    const existing = await prisma.informeEgreso.findFirst({
      where: { id: eid, informeId },
    });
    if (!existing) return NextResponse.json({ error: "Egreso no encontrado" }, { status: 404 });
    const updated = await prisma.informeEgreso.update({
      where: { id: eid },
      data,
    });
    return NextResponse.json({ ...updated, importe: Number(updated.importe) });
  } catch (err) {
    console.error("informe/[id]/egresos/[egresoId] PUT:", err);
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Error al actualizar egreso" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    if (!existing) return NextResponse.json({ error: "Egreso no encontrado" }, { status: 404 });
    await prisma.informeEgreso.delete({ where: { id: eid } });
    return new NextResponse(null, { status: 204 });
  } catch (err) {
    console.error("informe/[id]/egresos/[egresoId] DELETE:", err);
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Error al eliminar egreso" },
      { status: 500 }
    );
  }
}
